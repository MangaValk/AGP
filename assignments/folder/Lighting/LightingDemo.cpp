...
: D3DApp(hInstance), mBoxVB(0), mBoxIB(0), mLandVB(0), mLandIB(0), mWavesVB(0), mWavesIB(0),
  ...
  mInputLayout(0), mEyePosW(0.0f, 0.0f, 0.0f), mTheta(1.5f*MathHelper::Pi), mPhi(0.1f*MathHelper::Pi), mRadius(80.0f)
{
	...
	mPointLight.Ambient  = XMFLOAT4(0.0f, 0.0f, 1.0f, 1.0f);
	mPointLight.Diffuse  = XMFLOAT4(0.7f, 0.7f, 0.7f, 1.0f);
	mPointLight.Specular = XMFLOAT4(0.7f, 0.7f, 0.7f, 1.0f);
	mPointLight.Att      = XMFLOAT3(0.0f, 0.1f, 0.0f);
	mPointLight.Range    = 25.0f;

	/*mPointLight.Position.x = 70.0f / 5;
	mPointLight.Position.z = 70.0f / 5;
	mPointLight.Position.y = 20.0f;*/

	mPointLight.Position.x = 10;
	mPointLight.Position.y = 20.0f;
	mPointLight.Position.z = 10;
// 
//	// Point light--position is changed every frame to animate in UpdateScene function.
//	mPointLight2.Ambient  = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
//	mPointLight2.Diffuse  = XMFLOAT4(0.7f, 0.7f, 0.7f, 1.0f);
//	mPointLight2.Specular = XMFLOAT4(0.7f, 0.7f, 0.7f, 1.0f);
//	mPointLight2.Att      = XMFLOAT3(0.0f, 0.1f, 0.0f);
//	mPointLight2.Range    = 100.0f;
//
//	mPointLight2.Position.x = 10;
//	mPointLight2.Position.y = 15.0f;
//	mPointLight2.Position.z = 10;

	// Spot light--position and direction changed every frame to animate in UpdateScene function.
	mSpotLight.Ambient  = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
	mSpotLight.Diffuse  = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
	mSpotLight.Specular = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	mSpotLight.Att      = XMFLOAT3(1.0f, 0.0f, 0.0f);
	mSpotLight.Spot     = 96.0f;
	mSpotLight.Range    = 20.0f;

	mSpotLight.Position.x = -10;
	mSpotLight.Position.y = 30.0f;

//	// Spot light--position and direction changed every frame to animate in UpdateScene function.
//	mSpotLight2.Ambient  = XMFLOAT4(0.0f, 1.0f, 0.0f, 1.0f);
//	mSpotLight2.Diffuse  = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
//	mSpotLight2.Specular = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
//	mSpotLight2.Att      = XMFLOAT3(1.0f, 0.0f, 0.0f);
//	mSpotLight2.Spot     = 96.0f;
//	mSpotLight2.Range    = 100.0f;
//
//	mSpotLight2.Position.x = -10;
//	mSpotLight2.Position.y = 20.0f;
//	mSpotLight2.Position.z = -10;
...
void LightingApp::UpdateScene(float dt)
{
	...
	if( (mTimer.TotalTime() - t_base) >= 0.25f )
	{
		...
		if (mSpotLight.Range > 0)
			mSpotLight.Range = 0;
		else
			mSpotLight.Range = 10000.0f;

		/*if (mSpotLight.Spot > 0)
			mSpotLight.Spot -= 5;
		else
			mSpotLight.Spot = 96.0f;*/
	}
...
void LightingApp::DrawScene()
{
    ...
    for(UINT p = 0; p < techDesc.Passes; ++p)
    {
		...
		XMMATRIX boxOffset = XMMatrixTranslation(10.0f, 25.0f, 10.0f);
		XMStoreFloat4x4(&mLandWorld, XMMatrixMultiply(boxScale, boxOffset));
...
void LightingApp::BuildLandGeometryBuffers()
{
	...
	GeometryGenerator::MeshData wand;
	geoGen.CreateCylinder(2, 2, 20, 10, 1, wand);
	//geoGen.CreateBox(10.0f, 10.0f, 10.0f, wand);

	// Cache the vertex offsets to each object in the concatenated vertex buffer.
	mBoxVertexOffset = 0;

	// Cache the index count of each object.
	mBoxIndexCount = wand.Indices.size();

	// Cache the starting index for each object in the concatenated index buffer.
	mBoxIndexOffset = 0;

	UINT totalVertexCount = wand.Vertices.size();
	...
	UINT k = 0;
	for (size_t i = 0; i < wand.Vertices.size(); ++i, ++k)
	{
		vertices[k].Pos = wand.Vertices[i].Position;
		vertices[k].Normal = wand.Vertices[i].Normal;
		//vertices[k].Tex = wand.Vertices[i].TexC;
	}
	...
	indices.insert(indices.end(), wand.Indices.begin(), wand.Indices.end());
...
void LightingApp::BuildWaveGeometryBuffers()
{
	...
	vbd.ByteWidth = sizeof(Vertex2) * mWaves.VertexCount();
    vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
