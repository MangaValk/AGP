...
float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure) : SV_Target
{
	...
	if (gUseTexure)
	{
		...
		float4 resultColor = gDiffuseMap2.Sample(samAnisotropic, pin.Tex);
		float resultAlpha = gDiffuseMap2.Sample(samAnisotropic, pin.Tex).a;

		if (resultAlpha > 0)
			resultColor = gDiffuseMap2.Sample(samAnisotropic, pin.Tex);
		else
			resultColor = gDiffuseMap.Sample(samAnisotropic, pin.Tex);

		result = (resultColor);

		//result = (gDiffuseMap.Sample(samAnisotropic, pin.Tex) * gDiffuseMap.Sample(samAnisotropic, pin.Tex).a) + (gDiffuseMap2.Sample(samAnisotropic, pin.Tex) * gDiffuseMap2.Sample(samAnisotropic, pin.Tex).a);
